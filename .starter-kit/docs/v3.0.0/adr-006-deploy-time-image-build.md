...
The kit builds Docker images for the `webapp` and `async-job` Lambdas. The `dsql-migrator` is a zip-packaged NodejsFunction exception.
...
- **`dsql-migrator` exception**: The migrator is a zip-packaged `NodejsFunction`, not a deploy-time image build. CloudFormation synchronously invokes it during deployment, while container-image versions retain an initialization window that can raise the container-only `CodeArtifactUserPendingException`. `webapp` and `async-job` remain deploy-time-built images.
...
- **CodeBuild concurrent execution quota**: The concurrent execution quota for ARM/Small defaults to 1
  (for the entire AWS account). The two images in the same stack share one project through `SingletonProject`,
