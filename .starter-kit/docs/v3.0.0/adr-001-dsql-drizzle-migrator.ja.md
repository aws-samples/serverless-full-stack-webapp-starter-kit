...
- **マイグレーション状態管理**: `_migrations` テーブルは name（フルファイル名）+ executed_at のみで追跡。適用済みファイルの**内容ハッシュによる改竄検知**は不採用とした。フォーマッターやエディタが適用済みファイルを整形するとバイト列が変わり、ロジック無変更でも hash 不一致エラーになるため。適用済みファイルの改竄防止は git 管理で十分。これはデプロイ時再実行を駆動する zip asset hash（下記 C1）とは目的の異なる別機構である。
- **デプロイ時マイグレーション再実行の保証（C1）**: CDK Construct はランナーを `NodejsFunction` の zip asset としてパッケージし、`migrations/` ディレクトリ全体を asset root にコピーする。標準の CDK asset hash がマイグレーション変更を捉え、変更された asset は `currentVersion` の logical ID を変化させる。これにより CDK Trigger の version `HandlerArn` も変化し、最新ランナーが再実行される。「マイグレーション変更 → 新 asset/version → Trigger が最新を再実行」は `invalidateVersionBasedOn`、別のディレクトリ hash、またはカスタム `MigrationHash` Trigger property なしで保証される。コピー対象はランナー実行対象（`.sql`/`.mjs`）の上位集合（ディレクトリ全体）のままであり、migration metadata の変更は無害な追加再実行を生むだけである。
...
| hash 検証                 | なし                                 | 内容ハッシュ改竄検知は不採用（zip asset hash がデプロイ時の migration 変更を捉える） |
