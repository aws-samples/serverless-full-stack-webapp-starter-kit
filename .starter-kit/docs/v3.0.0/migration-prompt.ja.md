...
| `apps/db-migrator/`                               | **v3 で新設された workspace**（旧: `apps/cdk/lib/constructs/dsql-migrator/handler.ts` から `4149c22` で分離）。`package.json`, `tsconfig.json`, `src/handler.ts` 一式                                                                                                                   |
...
| `apps/cdk/lib/constructs/dsql-migrator/index.ts`  | zip パッケージの `NodejsFunction` + `Trigger`。esbuild が `migrations/` を asset にコピーし、通常の asset hash が version と Trigger の更新を駆動する。handler は `apps/db-migrator/` にある。                                                                       |
...
- **dsql-migrator Construct**（`apps/cdk/lib/constructs/dsql-migrator/index.ts`）と **`apps/db-migrator/` workspace**（handler）を追加（ただし Phase 5-1 まではデプロイしない）
- async-job の Lambda 関数定義を追加
- v2 の `prisma generate` や `prisma db push` に依存するビルドステップを削除
- **イメージビルドは deploy-time に CodeBuild で実行される**（`@cdklabs/deploy-time-build` の `ContainerImageBuild` construct、`393e96c`）。webapp / async-job の 2 イメージは同じ ARM64 CodeBuild プロジェクトを共有（`SingletonProject`）してビルドされ、dsql-migrator は esbuild によりローカルで zip パッケージ化される。ローカル `docker build` は必須ではないが、pnpm workspace + Docker の罠を synth 前に検出したい場合は 4-3b で任意に実施する
